...
void powermeter () { // Измеритель уровня выхода
  //fwdpin
  //revpin
  fwdpower = analogRead(fwdpin);
  revpower = analogRead(revpin);
  ...
  switch (menu) {
    ...
    case 0: //Если не в меню, то рисовать главный экран
      ...
      if (txen) {//Если передача, то вывод показометра мощности
        ...
        if ((fwdpower + revpower) / (fwdpower - revpower + 1) < 10)display.print(" ");
        display.print((fwdpower + revpower) / (fwdpower - revpower + 1));
        display.print(".");
        display.print(((fwdpower + revpower) % (fwdpower - revpower + 1)) % 10);
