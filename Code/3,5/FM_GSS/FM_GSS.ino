char ver[ ] = "FM_GSS";
/*
   ГСС 1 - 30МГц на основе Карат-3,5
   F0 - ГПД, F2 + VFO
   F2 - опора по дефолту 100МГц
...
#define bfo_max_freq 150000000UL // Максимальная частота F2 опоры, Гц.
...
struct general_set {
  ...
  uint32_t bfo_freq = 100000000UL; // Начальная частота BFO при первом включении.
...
void vfosetup() {
  if (band_setting.mode) {
    si.set_freq((band_setting.vfo_freq + general_setting.bfo_freq), 0, general_setting.bfo_freq);
  }
  else {
    si.set_freq((band_setting.vfo_freq + general_setting.bfo_freq), 0, general_setting.bfo_freq);
  }
