...
struct general_set {
  ...
  uint32_t usb_lo_freq = 13561590UL; // Начальная частота опоры USB при первом включении.
  uint32_t lsb_lo_freq = 13557900UL; // Начальная частота опоры LSB при первом включении.
  ...
  if (knobdown && !knobup) { //Если кноб нажат
    ...
    if (knobupmillis - knobMillis < 1000 && knobupmillis - knobMillis > 100) { //Если кноб отпущен и был нажат и времени от таймера прошло 100Мс
      if (menu < 4 && menu > 0 && actfmenuf) //Если 0<меню<4 и крутили енкодер в быстром меню, то выйти на главный экран
      {
        ...
        if (menu > 106) menu = 100; //Если меню больше 106 перейти на меню 100
      }
...
void bpfset() {
  //if (!bpfUpdate) {
    byte byteToSend = 0;
    byte bpf_set = 0;
    unsigned int bpf_freq = band_setting.vfo_freq / 1000000;
    if (bpf_freq >= b1) bpf_set = 1;
    if (bpf_freq >= b2) bpf_set = 2;
    if (bpf_freq >= b3) bpf_set = 3;
    if (bpf_freq >= b4) bpf_set = 4;
