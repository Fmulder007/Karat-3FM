...
#define fwdpin 15 // Порт прямой волны TX
#define revpin 16 // Порт обратной волны TX
...
//CW control pin
#define cwkeypin 11 // CW keyer pin MOSI
#define cwtxpin 13 // CW TX control pin SCK
...
struct general_set {
  ...
  byte cwdelay = 50; // Задержка переключения на прием после CW передачи * 10мсек
  byte cwtone = 70; // Сдвиг частоты CW *10 Гц
} general_setting;
...
unsigned int arraystp[] = {1, 10, 50, 100, 500, 1000, 10000}; //шаги настройки * 10 герц.

unsigned int myswr = 0;
unsigned int fwd = 0;
unsigned int rev = 0;
byte enc_div = 0;
byte mypower = 0;
...
// CW flags
boolean cwtxen = false;
boolean cwkeydown = false;
...
void setup() {
  //CW pin`s setup
  pinMode (cwkeypin, INPUT);
  digitalWrite (cwkeypin, HIGH);
  pinMode (cwtxpin, OUTPUT);
  digitalWrite (cwtxpin, LOW);
  // other pin`s setup
...
void powermeter () { // Измеритель уровня выхода
  ...
  rev = analogRead(revpin);
  mypower = map(fwd, 0, 1023, 0, 100);
  myswr = (fwd + rev) / (fwd - rev);
}
...
void cw() { // Процедура работы с ключом
  boolean keydown = digitalRead (cwkeypin); //Считываем состояние ключа
